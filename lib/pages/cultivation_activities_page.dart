...
class _CultivationActivitiesPageState extends State<CultivationActivitiesPage> {
  ...
  // Add this method to the _CultivationActivitiesPageState class:

  void _editCultivation(Cultivation cultivation) {
    if (widget.onNavigate != null) {
      widget.onNavigate!('add-cultivation', params: {
  ...
  Future<void> _deleteCultivation(String cultivationId, int index) async {
    final cultivation = _cultivations[index];

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Cultivation'),
        content: Text(
          'Are you sure you want to delete "${cultivation.cropName}"?\n\n'
              '⚠️ Warning: This will also delete ALL related activities and harvests for this cultivation.',
        ),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        await _firebaseService.deleteCultivation(cultivationId);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Cultivation deleted successfully'),
            backgroundColor: Colors.green,
          ),
        );
      } catch (e) {
        print('❌ Error deleting cultivation: $e');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error deleting cultivation: $e'),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      // Current crops list WITH SWIPE TO DELETE
                      if (!_isLoading && _cultivations.isNotEmpty)
                        Column(
                          children: _cultivations.asMap().entries.map((entry) {
                            final index = entry.key;
                            final cultivation = entry.value;
                            final daysUntilHarvest = cultivation.expectedHarvestDate
                                .difference(DateTime.now()).inDays;
                            final isOverdue = daysUntilHarvest < 0;

                            return Dismissible(
                              key: Key(cultivation.id),
                              ...
                              confirmDismiss: (direction) async {
                                if (direction == DismissDirection.startToEnd) { // ← FIXED: This should be startToEnd for LEFT to RIGHT
                                  // EDIT ACTION (swipe left to right)
                                  _editCultivation(cultivation);
                                  return false; // We handle navigation in the method
                                } else if (direction == DismissDirection.endToStart) { // ← This is RIGHT to LEFT
                                  // DELETE ACTION (swipe right to left)
                                  await _deleteCultivation(cultivation.id, index);
